...
/** 镜片画布只需要尺寸和 2d 上下文；保留 HTMLCanvasElement 的重载，单测里可直接传假对象。 */
export type MagnifierLensCanvas = Pick<HTMLCanvasElement, "getContext" | "height" | "width">;

/** 取正值兜底，避免用户传入 0 / 负数时出现除零或零尺寸画布。 */
 ...
 * 所以倍率和画板自身的缩放比例无关，用户看到的就是相对当前画面的放大效果。
...
export const getMagnifierSample = (
  ...
  const sourceLength = (lensSize / toPositive(zoom)) * ratio;
 ...
 * 把源画布的局部区域放大绘制到镜片画布上。
 *
 * 先铺白底：指针停在画板边缘时取样区域会超出源画布，drawImage 只画重叠部分，
...
export const drawMagnifierLens = (
  ...
  const deviceSize = Math.max(1, Math.round(lensSize * toPositive(pixelRatio)));
